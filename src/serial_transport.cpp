...
SerialTransport::SerialTransport(std::string port, int32_t baudrate)
    : m_port{port},
      m_baudrate{baudrate},
      m_timeout_us{150 * 1000},
      m_fd{0},
      m_timeoutFlag{false} {
}
...
bool SerialTransport::init() {
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "open %s %d", m_port.c_str(), m_baudrate);
  m_fd = ::open(m_port.c_str(), O_RDWR | O_NDELAY);
  if (m_fd < 0) {
    RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "open %s err", m_port.c_str());
    return false;
  }
  ...
  if (tcgetattr(m_fd, &opt) != 0) {
    RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "tcsetattr failed");
  }
...
Buffer SerialTransport::read() {
  ...
  if (retval == -1 && errno == EINTR) {
    RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "select failure");
    return data;
  }
  if (retval < 0) {
    RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "select failure");
